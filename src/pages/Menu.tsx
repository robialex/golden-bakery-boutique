...
const showcaseImages = [{
  src: showcasingBakes,
  alt: "Handcrafted pastries showcase"
}, {
  src: blueberry,
  alt: "Fresh blueberry dessert"
}, {
  src: heartPecan,
  alt: "Heart-shaped pecan cake"
}, {
  src: bdayCakes,
  alt: "Birthday celebration cakes"
}, {
  src: showcasingCakes,
...
const Menu = () => {
  ...
  const filteredCategories = productsByCategory.filter(categoryGroup => categoryGroup.name.toLowerCase().includes(searchTerm.toLowerCase()) || categoryGroup.products.some(p => p.name.toLowerCase().includes(searchTerm.toLowerCase())));
  ...
  const dynamicContent = useMemo(() => {
    ...
    filteredCategories.forEach((categoryGroup, index) => {
      // Add category card
      content.push(<motion.div key={categoryGroup.name} initial={{
        opacity: 0,
        y: 50,
        scale: 0.9
      }} whileInView={{
        ...
        ease: [0.22, 1, 0.36, 1]
          ...
          <CategoryPreview category={categoryGroup.name} products={categoryGroup.products} index={index} />
        </motion.div>);
      cardPairCount++;
      ...
      if (cardPairCount === 2 && showcaseIndex < showcaseImages.length) {
        content.push(<div key={`showcase-${showcaseIndex}`} className="col-span-2 lg:col-span-4">
            <FullWidthShowcase imageSrc={showcaseImages[showcaseIndex].src} alt={showcaseImages[showcaseIndex].alt} />
          </div>);
        showcaseIndex++;
        cardPairCount = 0;
      }
      ...
      if (index > 0 && (index + 1) % 4 === 0 && ctaIndex < ctaBlocks.length) {
        content.push(<div key={`cta-${ctaIndex}`} className="col-span-2 lg:col-span-4">
            <SpecialtyFocusBlock imageSrc={ctaBlocks[ctaIndex].imageSrc} headline={ctaBlocks[ctaIndex].headline} bodyText={ctaBlocks[ctaIndex].bodyText} ctaText={ctaBlocks[ctaIndex].ctaText} ctaLink={ctaBlocks[ctaIndex].ctaLink} />
          </div>);
        ctaIndex++;
      }
    });
      ...
      <div className="container relative z-10 my-[20px] mx-0 px-[17px] py-0">
        ...
        ease: "easeOut"
          ...
          delay: 0.4
            ...
            <Link to="/menu/vegetarian" className="inline-flex items-center gap-2 px-6 py-3 bg-[#C6A136] text-white rounded-xl font-semibold shadow-[0_4px_12px_rgba(198,161,54,0.25)] hover:shadow-[0_0_12px_rgba(198,161,54,0.5)] hover:scale-[1.03] transition-all duration-300">
